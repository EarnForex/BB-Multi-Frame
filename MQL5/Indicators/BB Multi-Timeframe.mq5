...
int OnInit()
{
    ...
    // Prepare BB indicator handles.
    for (int i = 0; i < ArraySize(TFValues); i++)
    {
        if (!TFEnabled[i]) continue;

        BB_Handles[i] = iBands(Symbol(), TFValues[i], BBPeriod, BBShift, BBDeviation, BBAppliedPrice);
    }
...
void CalculateLevels()
{
    ...
    for (int i = 0; i < ArraySize(TFValues); i++)
    {
        ...
        {
            double Buf[1];
            int n = CopyBuffer(BB_Handles[i], 0, j, 1, Buf);
            if (n < 1)
            {
                Print("BB middle line data not ready for ", Symbol(), " @ ", EnumToString(TFValues[i]));
                return;
            }
            double BB_Main = Buf[0];

            n = CopyBuffer(BB_Handles[i], 1, j, 1, Buf);
            if (n < 1)
            {
                Print("BB upper band data not ready for ", Symbol(), " @ ", EnumToString(TFValues[i]));
                return;
            }
            double BB_Upper = Buf[0];
            
            n = CopyBuffer(BB_Handles[i], 2, j, 1, Buf);
            if (n < 1)
            {
                Print("BB lower band data not ready for ", Symbol(), " @ ", EnumToString(TFValues[i]));
                return;
            }
            double BB_Lower = Buf[0];
...
void Notify()
{
    ...
    if (SendAlert)
    {
        string AlertText = "BB MTF Signal (" + SignalString + "): " + SituationString + ".";
        Alert(AlertText);
    }
    if (SendEmail)
    {
